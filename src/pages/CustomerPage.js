    ...
    <div className="container pb-5">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 mt-3">
        ...
        <div className="d-flex gap-2">
          ...
          {!isEditing && (
              <button 
                className={`btn ${showLoanForm ? 'btn-secondary' : 'btn-success'}`} 
                onClick={() => setShowLoanForm(!showLoanForm)}
              >
                <i className={`bi ${showLoanForm ? 'bi-x-lg' : 'bi-plus-circle'} me-2`}></i>
                {showLoanForm ? 'Cancel Pledge' : 'Create New Pledge'}
              </button>
          )}
          
          <button className="btn btn-warning" onClick={() => { setIsEditing(!isEditing); setShowLoanForm(false); }}>
            <i className="bi bi-pencil me-1"></i> {isEditing ? 'Cancel Edit' : 'Edit Profile'}
          </button>
          
          {/* --- RESTORED DELETE BUTTON TEXT --- */}
          {userRole === 'admin' && (
      ...
      {/* Conditional Render: Edit Customer Form */}
      {isEditing ? (
        <EditCustomerForm customer={customer} onUpdateSuccess={handleUpdateSuccess} onCancel={() => setIsEditing(false)} />
      ) : (
        <>
            {/* Conditional Render: Create Loan Form */}
            {showLoanForm && (
                <div className="mb-4 border-bottom pb-4">
                    <LoanForm customerId={id} onLoanAdded={handleLoanAdded} />
                </div>
            )}
            ...
            <div className="col-md-4 mb-3">
                ...
                <div className="card-body text-center p-4">
                    ...
                    <div className="text-start mt-4 pt-3 border-top">
                        <h6 className="text-uppercase text-primary small fw-bold mb-3">Additional Details</h6>
                        
                        {customer.id_proof_number && (
                            <div className="mb-2 d-flex justify-content-between">
                                <span className="text-muted small">ID ({customer.id_proof_type}):</span>
                                <span className="fw-medium small">{customer.id_proof_number}</span>
                            </div>
                        )}
                        
                        {customer.nominee_name && (
                            <div className="mb-0 d-flex justify-content-between">
                                <span className="text-muted small">Nominee ({customer.nominee_relation}):</span>
                                <span className="fw-medium small">{customer.nominee_name}</span>
                            </div>
                        )}
                    </div>
                    )}
